...
> R:Hacer un solo commit grande dificulta el seguimiento de cambios, ya que no permite identificar fácilmente qué modificación introdujo un error. Además, complica revertir cambios puntuales y hace más difícil la revisión del código, ya que mezcla cambios sin relación. Es mejor hacer commits pequeños y con propósito claro

**P4** — ¿Cuál es la diferencia entre `git fetch` y `git pull`? ¿Cuándo preferirías usar uno sobre el otro?

> R:git fetch descarga los cambios del repositorio remoto pero no los integra en la rama actual, permitiendo revisar antes de aplicar cambios. En cambio, git pull descarga e integra automáticamente esos cambios en un solo paso. Se prefiere fetch cuando se quiere tener mayor control sobre qué cambios se integran, y pull cuando se quiere actualizar rápidamente la rama local.

**P5** — ¿Qué información debería tener la descripción de un Pull Request para que sea útil para quien lo revisa?

> R:Un Pull Request debería incluir una descripción clara del problema que resuelve o la funcionalidad que agrega, cómo se probó que funciona correctamente y cualquier decisión de diseño relevante. Esto facilita la revisión y comprensión del cambio por parte de otros desarrolladores.
>>>>>>> origin/feature/conflicto-demo
